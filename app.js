...
app.post('/movies', async (req, res) => { //Defines a POST route at /movies using Express. The async keyword indicates that the function will handle asynchronous operations and use await for managing promises.
    ...
    } catch (err) { //If an error occurs, it sends a 500 Internal Server Erro
...
app.put('/movies/:id', async (req, res) => { //Defines a PUT route at /movies/:id using Express. The :id part of the URL is a route parameter that captures the movie's ID from the request URL.
    ...
    try { //handle potential errors
        ...
        const movie = movieResult.rows[0]; //Stores the movie record retrieved from the database into the movie variable. movieResult.rows[0] contains the first (and only) row returned by the query.

        let actor = null; //Checks if an actorId is provided in the request body.
        if (actorId) { //If actorId is provided
            actor = await findActorById(actorId); //it uses the findActorById function to check if an actor with that ID exists in the database. The await keyword waits for the result of the findActorById function.
            if (!actor) { //If no actor is found (i.e., actor is null or undefined), it sends a 404 Not Found response
        ...
        const updatedMovie = await pool.query( // Executes an UPDATE SQL query using pool.query to update the movie record in the movies table.
            ...
            //title || movie.title means if title is provided, use it; otherwise, keep the existing title.
            //creationDate || movie.creation_date means if creationDate is provided, use it; otherwise, keep the existing creation date.
            //actorId || movie.actor_id means if actorId is provided, use it; otherwise, keep the existing actor ID.
            //req.params.id is the ID of the movie being updated.
        );
